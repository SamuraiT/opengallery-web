...
var View = Backbone.View.extend({
  initialize: function(options) {
    ...
    this.listenTo(this.collection, 'select', function (model) {
      ...
      var data = _.extend(model.toJSON(), {selected: 'selected'});
      exhibitions[index] = data;
      this.list.set('exhibitions', exhibitions);
  ...
  events: {
    'click .ex-item:not(.selected)': 'selectHandler',
    'click .ex-item.selected': 'closeContent',
    'click .ex-content-close': 'closeContent'
  },
  set: function() {
    this.list.set.apply(this.list, arguments);
  },
  setSelected: function() {
    this.content.set.apply(this.content, arguments);
  },
